...
class ForgotPassword extends Component {
  ...
  sendOTP = () => {
    this.setState({
      generatedOTP: Math.floor(Math.random() * 1000000), //6 digit otp generated
    });
    //Get username
    axios
          ...
          if (response.data[0].user_type == 0) {
            axios
              ...
              .then((resp) => {
                ...
                console.log(body);
                axios
                  .post("http://localhost:5000/users/sendmail", body)
                  .then((re) => {
  ...
  changePassword = () => {
    ...
    axios
      .post("http://localhost:5000/users/resetPassword", body)
      .then((response) => {
      ...
      <Container className="toplookout">
        ...
        {!this.state.sendotp && (
          <React.Fragment>
            ...
            <Button onClick={this.sendOTP} style={{ marginBottom: "20px" }}>
              Send OTP
            </Button>
          </React.Fragment>
        )}
        ...
        <Collapse isOpen={this.state.sendotp}>
          {!this.state.enable_password && (
            <React.Fragment>
              <p>
                An OTP has been mailed to <b>{this.state.email}</b>
              </p>
              ...
              <Button color="success" onClick={this.checkOTP}>
                Submit
              </Button>
            </React.Fragment>
          )}
