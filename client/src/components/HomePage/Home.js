...
class Home extends Component {
  ...
  refresh = () => {
    this.props.fetchHome(this.props.user_id, this.props.is_mentor);
  };
  ...
  delete_pending = (request_id, mentor_id) => {
    ...
    axios
      .post("http://localhost:5000/session/delete_pending", body)
      .then((res) => {
  ...
  render() {
      ...
      <React.Fragment key={up.session_id}>
        <Container className="spaceout-tabs-contents">
          <Row>
            <Col>
              <h4>
                <b>{this.props.all_skills[up.skill_id - 1].skill_name}</b>{" "}
                - {up.topic}
              </h4>
              <Row>
                <Col sm={4}>
                  <h5>
                    📅{" "}
                    {new Date(up.req_date).getDate() +
                      "-" +
                      (new Date(up.req_date).getMonth() + 1) +
                      "-" +
                      new Date(up.req_date).getFullYear()}
                  </h5>
                </Col>
                <Col sm={4}>
              ...
              {!this.props.is_mentor ? (
                <React.Fragment>
                  <h5>
                    <b>Mentor:</b> {up.first_name} {up.last_name},{" "}
                    {up.qualification}
                  </h5>
                </React.Fragment>
              ) : (
                <React.Fragment>
                  <h5>
                    <b>Student:</b> {up.first_name} {up.last_name},{" "}
                    {up.qualification}
                  </h5>
                  <h5>
                    <b>Graduation:</b> {up.grad_year}, {up.college_name}
                  </h5>
                </React.Fragment>
              )}
              {!this.props.is_mentor && (
    ...
    const pending_mentors = this.props.is_mentor
      ? this.props.pending_requests.map((req) => (
          <React.Fragment key={req.request_id}>
            <div className="spaceout-tabs-contents">
              <Button
                ...
                style={{ float: "right" }}
                onClick={() =>
                  this.delete_pending(req.request_id, this.props.user_id)
                }
              >
              ...
              <Row>
                <Col sm={9}>
                  <h4>
                    <b>
                      {
                        this.props.all_skills[parseInt(req.skill_id) - 1]
                          .skill_name
                      }
                    </b>{" "}
                    - {req.topic}
                  ...
                  <Row>
                    <Col sm={4}>
                      <h5>
                        📅{" "}
                        {new Date(req.req_date).getDate() +
                          "-" +
                          (new Date(req.req_date).getMonth() + 1) +
                          "-" +
                          new Date(req.req_date).getFullYear()}
                      </h5>
                    </Col>
                    <Col sm={4}>
                    ...
                    <Col sm={4}>
                      <h5>
                        🔡{" "}
                        {
                          this.props.all_languages[
                            parseInt(req.language_id) - 1
                          ].language_name
                        }
                      </h5>
                    </Col>
                  ...
                  <br />
                  <h5>
                    <b>Student:</b> {req.user.first_name} {req.user.last_name},{" "}
                    {req.user.qualification}
                  </h5>
                  <h5>
                    <b>Graduation:</b> {req.user.grad_year}, {req.user.college_name}
                  </h5>
                </Col>
                <Col sm={3}>
                  ...
                  <h6>
                    ...
                    {req.final === 0 && req.approved === 0 && (
                      <span style={{ color: "green" }}>
                        Student hasn't confirmed any mentor yet
                      </span>
                    )}
                    {req.final === 0 && req.approved === 1 && (
                      <span style={{ color: "red" }}>
                        Student has chosen another mentor
                      </span>
                    )}
                  </h6>
                </Col>
    ...
    const pending = !this.props.is_mentor
      ? this.props.pending_requests.map((req) => (
          <React.Fragment key={req.request_id}>
            <div className="spaceout-tabs-contents">
              <h4>
              ...
              <Row>
                ...
                <Col sm={4}>
                  <h5>
                    🔡{" "}
                    {
                      this.props.all_languages[parseInt(req.language_id) - 1]
                        .language_name
                    }
                  </h5>
                </Col>
              ...
              req.mentor_specific === -1 ? (
                ...
                <Row>
                  <h5>This request is mentor specific</h5>
                </Row>
              )}
            </div>
          </React.Fragment>
                  ...
                  <h4>
                    {this.props.is_mentor ? (
                      <b>
                        🎖️Your Rating:{" "}
                        {up.review === 0 ? (
                          <span>Student didn't give a rating</span>
                        ) : (
                          <span>{up.review}/5</span>
                        )}
                      </b>
                    ) : (
                      <b>
                        🎖️You Rated:{" "}
                        {up.review === 0 ? (
                          <span>You didn't give any review</span>
                        ) : (
                          <span>{up.review}/5</span>
                        )}
                      </b>
          ...
          <React.Fragment>
            <Container>
              <Col style={{ textAlign: "left" }}>
                {this.props.LoggedIn && !this.props.is_mentor && (
                  <ReqSession />
                )}
                {this.props.LoggedIn && this.props.is_mentor && (
                  <React.Fragment>
                    {this.props.verfied === 1 ? (
                      <main role="main">
                        <div className="row d-flex justify-content-center">
                          ...
                          >
                            <section id="prompt">
                              <h3>
                                Help curious students get the best out of their
                                education!
                              </h3>
                              <br />
                              <p>
                                Check if there are any requests for sessions in
                                the skills of your expertise, and accept if
                                you would like to conduct the session.
                              </p>
                            </section>
                      ...
                      <main role="main" className="text-center">
                        ...
                        <p>
                          You will be able to participate once the administrator
                          verifies your profile.
                          <br />
                          If you think it has already been verified, try
                          refreshing the page.
                        </p>
                      </main>
                    ) : (
                      <main role="main" className="text-center">
                        ...
                        <br />
                        <h3>
                          Sorry {this.props.first_name} {this.props.last_name},
            ...
            {this.props.LoggedIn && (
              <React.Fragment>
                {((this.props.is_mentor && this.props.verfied === 1) ||
                  !this.props.is_mentor) && (
                  <React.Fragment>
                    <ButtonGroup>
                      {this.state.show_upcom ? (
                        <Button color="success" onClick={this.showupcoming}>
                          Upcoming Sessions
                        </Button>
                      ) : (
                      ...
                      {this.state.show_pend ? (
                        <Button color="success" onClick={this.showpending}>
                          Pending Requests
                        </Button>
                      ) : (
                      ...
                      {this.state.show_past ? (
                        <Button color="success" onClick={this.showpast}>
                          Past Sessions
                        </Button>
                      ) : (
                      ...
                      <Row>
                        ...
                        {this.state.show_pend &&
                          (this.props.pending_requests.length === 0 ? (
                            ...
                            <Container>{pending_mentors}</Container>
                          ) : (
