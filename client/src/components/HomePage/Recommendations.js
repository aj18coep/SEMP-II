...
//Give mentor recommendations to student
  ...
  render() {
    const recommens = this.state.recomms.map((rec) => (
      <React.Fragment key={rec.user_id}>
        <Container
          className="text-left"
          style={{ background: "#f8f0fd", padding: 30 }}
        >
          <Row>
            <Col sm={3}>
          ...
          <React.Fragment>
            ...
            {this.state.recomms.length === 0 ? (
              <div className="text-left">
                <h5>
                  Add your favorite skills to your profile so we can recommend
                  mentors!
                </h5>
