      ...
      bcrypt.compare(
        ...
        (error, response) => {
          if (response) {
            ...
            res.send({
              message: "Password and username combination doesn't match",
            });
          }
        }
      );
...
router.post(
  ...
  async function (req, res, next) {
    ...
    if (req.file === null) {
      ...
      var def = false;
      if (
        req.body.oldurl.localeCompare(
          "http://localhost:5000/profile_pics/default.png"
        ) === 0
      )
        def = true;
      try {
        ...
        await pipeline(
          ...
          fs.createWriteStream(
            `${__dirname}/../public/profile_pics/${filename}`
          )
        );
